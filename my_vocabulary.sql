...
BEGIN
  ...
  INSERT INTO vocabulary(english_word, foreign_word, foreign_id, datein, last_updated) VALUES(pEnglish_word, pForeign_word, pForeign_language, CURDATE(), CURDATE());
...
BEGIN
    ...
    ELSEIF (letter LIKE 'Rec') THEN
      SELECT COUNT(*) FROM vocabulary v, foreign_language f WHERE v.foreign_id = f.id AND f.language = pForeign_language INTO total;
      SELECT english_word, foreign_word FROM vocabulary v, foreign_language f WHERE v.foreign_id = f.id AND f.language = pForeign_language ORDER BY v.id DESC;
    ELSE
...
BEGIN
  ...
  CASE word_language
    WHEN 0 THEN
      UPDATE vocabulary INNER JOIN foreign_language ON foreign_language.id = vocabulary.foreign_id SET english_word = pEnglish_word, last_updated = CURDATE() WHERE foreign_word = pForeign_word AND language = pForeign_language;
    WHEN 1 THEN
      UPDATE vocabulary INNER JOIN foreign_language ON foreign_language.id = vocabulary.foreign_id SET foreign_word = pForeign_word, last_updated = CURDATE() WHERE english_word = pEnglish_word AND language = pForeign_language;
    ELSE
      /* DO NOTHING */SELECT 1;
    END CASE;
